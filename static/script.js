...
function calculateDistance(lat1, lon1, lat2, lon2) {
  ...
  console.log(`Jarak: ${distance.toFixed(2)} km`); // Debugging: lihat hasil perhitungan jarak
  return distance;
}
...
function getUserLocation(callback) {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      console.log(`Lokasi Pengguna: ${userLat}, ${userLon}`); // Debugging: melihat lokasi pengguna
    ...
    .then((data) => {
      ...
      getUserLocation((userLat, userLon) => {
        schools = schools.map((school) => {
          ...
          // Membuat link rute untuk membuka Google Maps dengan titik awal (user) dan tujuan (school)
          school.mapLink = `https://www.google.com/maps/dir/${userLat},${userLon}/${school.latitude},${school.longitude}`;
